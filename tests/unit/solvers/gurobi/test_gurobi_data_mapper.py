...
def test_define_variables_creates_edge_and_face_vars(gurobi_data_mapper, triangulation_problems):
    for problem in triangulation_problems:
        model = Model()
        gurobi_data_mapper.define_variables(model, problem)
        ...
        assert set(model._edge_vars.keys()) == set(problem.candidate_edges)
        assert set(model._face_vars.keys()) == set(problem.candidate_faces)
...
def test_to_gurobi_model_returns_model_with_vars(gurobi_data_mapper, triangulation_problems):
    for problem in triangulation_problems:
...
def test_set_objective_calls_supported_function(gurobi_data_mapper, triangulation_problems):
    for problem in triangulation_problems:
        model = Model()
        gurobi_data_mapper.define_variables(model, problem)
        gurobi_data_mapper.set_objective(model, problem)

def test_apply_constraints_skips_unknown(gurobi_data_mapper, triangulation_problems):
    for problem in triangulation_problems:
        model = Model()
        gurobi_data_mapper.define_variables(model, problem)
        gurobi_data_mapper.apply_constraints(model, problem)
